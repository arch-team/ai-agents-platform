...
```python
# ✅ 允许：ORM 模型定义外键关系
# modules/{module}/infrastructure/models/{entity}_model.py
from {PROJECT}.modules.auth.infrastructure.models import UserModel
...
```python
from {PROJECT}.shared.domain import PydanticEntity
...
class {Entity}Repository(PydanticRepository[{Entity}, {Entity}Model, int], I{Entity}Repository):
...
| 服务 | `{entity}_service.py` | `user_service.py` |
