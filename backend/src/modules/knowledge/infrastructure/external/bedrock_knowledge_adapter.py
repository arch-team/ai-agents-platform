...
class BedrockKnowledgeAdapter(IKnowledgeService):
    ...
    async def create_knowledge_base(self, name: str, *, s3_bucket: str, s3_prefix: str) -> KBCreateResult:  # noqa: ARG002, D102
    ...
    async def delete_knowledge_base(self, bedrock_kb_id: str) -> None:  # noqa: D102
        try:
    ...
    async def start_sync(self, bedrock_kb_id: str) -> KBSyncResult:  # noqa: D102
        try:
            resp = await asyncio.to_thread(
    ...
    async def retrieve(self, bedrock_kb_id: str, query: str, *, top_k: int = 5) -> list[RetrievalChunk]:  # noqa: D102
        try:
            resp = await asyncio.to_thread(
