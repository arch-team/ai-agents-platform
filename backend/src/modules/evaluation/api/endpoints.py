...
async def create_test_suite(
    request: CreateTestSuiteRequest, service: SuiteServiceDep, current_user: CurrentUserDep,
) -> TestSuiteResponse:
    """创建测试集。"""
...
async def list_test_suites(
    service: SuiteServiceDep, current_user: CurrentUserDep,
...
async def update_test_suite(
    suite_id: int, request: UpdateTestSuiteRequest, service: SuiteServiceDep, current_user: CurrentUserDep,
) -> TestSuiteResponse:
    """更新测试集。"""
...
async def activate_test_suite(
    suite_id: int, service: SuiteServiceDep, current_user: CurrentUserDep,
) -> TestSuiteResponse:
    """激活测试集。"""
...
async def add_test_case(
    suite_id: int, request: CreateTestCaseRequest, service: SuiteServiceDep, current_user: CurrentUserDep,
) -> TestCaseResponse:
    """添加测试用例到测试集。"""
...
async def delete_test_case(
    suite_id: int, case_id: int, service: SuiteServiceDep, current_user: CurrentUserDep,
) -> None:
    """删除测试用例。"""
...
async def create_evaluation_run(
    request: CreateEvaluationRunRequest, service: EvalServiceDep, current_user: CurrentUserDep,
) -> EvaluationRunResponse:
    """执行评估运行。"""
...
async def list_evaluation_runs(
    service: EvalServiceDep, current_user: CurrentUserDep,
...
async def get_evaluation_results(
    run_id: int, service: EvalServiceDep,
    page: Annotated[int, Query(ge=1)] = 1, page_size: Annotated[int, Query(ge=1, le=100)] = 20,
) -> EvaluationResultListResponse:
    """获取评估运行的结果列表。"""
