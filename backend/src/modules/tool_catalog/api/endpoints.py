...
from src.modules.tool_catalog.application.dto.tool_dto import CreateToolDTO, ToolDTO, UpdateToolDTO
...
async def create_tool(request: CreateToolRequest, service: ServiceDep, current_user: CurrentUserDep) -> ToolResponse:
...
async def delete_tool(tool_id: int, service: ServiceDep, current_user: CurrentUserDep) -> None:
    """删除 Tool。仅 creator 可操作，仅 DRAFT 状态可删除。"""
    await service.delete_tool(tool_id, current_user.id)


@router.post("/{tool_id}/submit")
async def submit_for_review(tool_id: int, service: ServiceDep, current_user: CurrentUserDep) -> ToolResponse:
...
async def approve_tool(tool_id: int, service: ServiceDep, current_user: AdminUserDep) -> ToolResponse:
...
async def deprecate_tool(tool_id: int, service: ServiceDep, current_user: CurrentUserDep) -> ToolResponse:
