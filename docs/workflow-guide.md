...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│  1. 任务拆解  │ ──► │  2. 逐项实现  │ ──► │  3. 质量验收  │ ──► │  4. 推进下一个 │
│  (场景 1)     │     │  (场景 2 × N) │     │  (场景 3)     │     │  Milestone    │
└──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘
    读取 roadmap         读取 progress          运行检查工具          更新 progress
    拆解为任务           TDD 实现每个任务        对照 checklist       归档已完成 Milestone
    写入 progress        更新任务状态            对照 roadmap 验收     拆解新 Milestone
```
