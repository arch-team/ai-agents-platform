...
export function useSendMessageStream() {
  ...
  const sendMessage = useCallback(
    async (conversationId: number, dto: SendMessageDTO) => {
      ...
      try {
        const token = (window as unknown as Record<string, unknown>).__AUTH_TOKEN__ as
          | string
          | undefined;
        const url = `${env.VITE_API_BASE_URL}/api/v1/conversations/${conversationId}/messages/stream`;

        for await (const chunk of streamSSE(url, dto, token ?? null)) {
          if (chunk.error) {
    ...
    [appendStreamContent, clearStream, setStreaming, queryClient],
  );
